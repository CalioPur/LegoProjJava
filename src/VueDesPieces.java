	...
	public void paint(Graphics g) {
		...
		if (brk != null) {
			...
			if (angle == 9) {
				//face du dessus
				int x1 = (int) ((int)sizeX/2 - (((brk.largeurBrique+brk.longueurBrique)/2 * 30 *Math.sqrt(3)) / 2));
				int y1 = (int)sizeY/2 - ((brk.longueurBrique * 30) / 2)-50;
				int x2 = (int) (brk.largeurBrique * 30 *Math.sqrt(3));
				int y2 = brk.largeurBrique * 30;
				int x3 = (int) (brk.longueurBrique * 30 *Math.sqrt(3));
				int y3 = brk.longueurBrique * 30;
				int z1=brk.hauteurBrique * 30;
				if (etat!=0) {
					x1=(int)(sizeX);
					x2=(int)(x2*0.65);
